...
export default class InfiniteScroll extends React.Component {
  ...
  dimension_width = Dimensions.get('window').height;
  ...
  translateX_array = [
    ...
    new Animated.Value(this.dimension_width),
    new Animated.Value(-this.dimension_width),
  ];
  ...
  _onGestureEvent = Animated.event([{}], {
    ...
    listener: ({nativeEvent}) => {
      ...
      this.record_translateX_array.forEach((translate, index, arr) => {
        ...
        arr[index] += nativeEvent.translationY - this.old_translateX;
      });
      ...
      this.old_translateX = nativeEvent.translationY;
      ...
      if (
        this.record_translateX_array[this.main_index] >= -120 &&
        this.record_translateX_array[this.main_index] <= 120
      ) {
      ...
      this.opacity_array[this.main_index].setValue(
        (this.dimension_width -
          Math.abs(this.record_translateX_array[this.main_index])) /
          (this.dimension_width * 1.3),
      );
    },
  });
  ...
  handleAnimation = main_index => {
    ...
    if (
      this.record_translateX_array[main_index] >= -120 &&
      this.record_translateX_array[main_index] <= 120
    ) {
      ...
      this.record_translateX_array[(main_index - 1) % 3] = -this
        .dimension_width;
      ...
      Animated.parallel(animation_array, {
        stopTogether: true,
      }).start();
    }
    ...
    // if the translation of the present component is lower than the left limit => we swipe to the left => go to the future calendars.
      ...
      this.translateX_array[(main_index - 1) % 3].setValue(
        this.dimension_width,
      );
      ...
      let animation_array = this.translateX_array.map((translate, index) => {
        if (index !== (main_index - 1) % 3)
          return Animated.spring(this.translateX_array[index], {
            toValue: this.record_translateX_array[index],
          });
      });
      ...
      <View
        ...
        <PanGestureHandler
          ...
          <Animated.View
            style={{
              flex: 1,
              alignItems: 'center',
              transform: [{translateY: this.translateX_array[0]}],
            ...
            /> */}
            <Text style={{fontSize: 60}}>{DATA[0].key}</Text>
        ...
        <PanGestureHandler
          ...
          <Animated.View
            style={{
              flex: 1,
              transform: [{translateY: this.translateX_array[1]}],
              backgroundColor: 'red',
            }}>
            <Calendar
        ...
        <PanGestureHandler
          ...
          <Animated.View
            style={{
              flex: 1,
              transform: [{translateY: this.translateX_array[2]}],
              position: 'absolute',
  ...
  render() {
    return (
      <View
        ...
        <MonthHolder month_data_array={this.state.month_data_array} />
      </View>
    );
  }
}
...
class MonthHolder extends React.Component {
  ...
  componentDidUpdate(prevProps, prevState) {
    if (this.props.month_data_array !== prevProps.month_data_array) {
      let diff =
        this.props.month_data_array[0].day_in_week === 0
          ? 6
          : this.props.month_data_array[0].day_in_week - 1;

      let fill_array = [];

      for (let i = 0; i < diff; i++) {
        fill_array.push(
          <View key={`dummy-${i}`} style={styles.notChosen}></View>,
        );
      }
...
class Day extends React.Component {
  ...
  shouldComponentUpdate(nextProps, nextState) {
    return (
      (nextProps.day_index === nextProps.current_day_index &&
        nextProps.month_index === nextProps.current_month_index) ||
      (nextProps.day_index === nextProps.last_day_index &&
        nextProps.month_index === nextProps.current_month_index) ||
      (nextProps.day_index === nextProps.last_day_index &&
